...
export default function PrivacyPage() {
  const { data, loading, error, reload } = useAdminFetch<CmsPage>('/admin/pages/type/privacy');
  const { saving, error: crudError, update, clearError } = useCrudActions();
  const [open, setOpen] = useState(false);

  return (
    <div>
      <PageHeader title="سياسة الخصوصية" description="تحرير المحتوى بثلاث لغات">
        {data && (
          <button
            type="button"
            onClick={() => {
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="تعديل سياسة الخصوصية" wide>
        {data && (
          <LegalPageForm
